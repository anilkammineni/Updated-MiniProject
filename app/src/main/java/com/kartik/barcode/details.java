    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        b1.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                check=FirebaseDatabase.getInstance().getReference().child("All Barcodes");
                check.orderByChild("Barcode").equalTo(temp).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if(dataSnapshot.exists())
                        {
                            get=FirebaseDatabase.getInstance().getReference().child("All Barcodes").child(temp);
                            get.addValueEventListener(new ValueEventListener() {
                                @Override
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    String product=dataSnapshot.child("Pname").getValue().toString();
                                    Toast.makeText(details.this,"Product Name is "+product,Toast.LENGTH_LONG).show();
                                }
                        ...
                        {
                            Toast.makeText(details.this,"No product is available",Toast.LENGTH_LONG).show();
                        }
                    }
